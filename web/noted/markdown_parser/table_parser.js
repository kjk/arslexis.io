 ...
 * @param {string} line
 * @param {number} startI
 * @param {import("@lezer/markdown").Element[]} elts
 * @param {number} offset
 * @returns
 */
function parseRow(cx, line, startI = 0, elts = undefined, offset = 0) {
  let count = 0,
    first = true,
    cellStart = -1,
    cellEnd = -1,
...
/**
 * @implements {LeafBlockParser}
 */
class TableParser {
  // Null means we haven't seen the second line yet, false means this
  // isn't a table, and an array means this is a table and we've
  // parsed the given rows so far.
  /** @type {false | null | Element[]} */
  rows = null;

  /**
   * param {BlockContext} cx // TODO: confused about elt() having arg of type HTML Element vs Markdown Element
   * @param {Line} line
   * @param {LeafBlock} leaf
   * @returns
   */
